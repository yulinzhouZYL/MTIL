...
class MambaPolicy(nn.Module):
    ...
    def process_vision_chunk(self, dino_feats):
        ...
        # 根据分辨率选择 adapter
        if self.img_size == (640, 480):
            return self.spatial_adapter(dino_feats)
        elif self.img_size == (128, 128):
            return self.spatial_adpater_low(dino_feats)
        else:
            # Fallback or error
            return self.spatial_adapter(dino_feats)

    def forward_sequence(self, obs_lowdim, cam_features_seq):
        """
        全序列并行前向传播。
        
        Args:
            obs_lowdim: [B, L, 14]
            cam_features_seq: Dict {cam_name: [B, L, embed_dim]} 
                              注意：这里输入的是已经过 Adapter 的特征序列！
        
        Returns:
            pred_actions: [B, L, future_steps, action_dim]
        """
        B, L, _ = obs_lowdim.shape
        device = obs_lowdim.device
        
        # 1. 融合多相机特征 [B, L, num_cam * embed_dim]
        feats_all = []
        for cam in self.camera_names:
            if cam in cam_features_seq:
                feats_all.append(cam_features_seq[cam])
            else:
                # Padding 情况下可能有些 cam 没有？通常应该都有
                feats_all.append(torch.zeros(B, L, self.embed_dim, device=device))
        
        if len(feats_all) > 0:
            cam_feats = torch.cat(feats_all, dim=2) # [B, L, N*D]
        else:
            cam_feats = torch.zeros(B, L, self.embed_dim, device=device) # Fallback

        # 2. 展平以便进行帧级别的 Cross Attention
        # 我们需要把 [B, L] 视作 Batch 维度 [B*L] 进行模态融合
        cam_feats_flat = cam_feats.view(B * L, -1) # [B*L, in_dim]
        lowdim_flat = obs_lowdim.view(B * L, -1)   # [B*L, 14]

        # 3. Cross Camera Attention (如果你有多相机)
        if self.num_cameras > 1 and hasattr(self, 'cross_cam_attn'):
            # Attention expects [Seq, Batch, Dim]
            inp = cam_feats_flat.unsqueeze(0) # [1, B*L, Dim]
            out = self.cross_cam_attn(inp, inp, inp)
            cam_feats_flat = out.squeeze(0)

        # 4. Modal Fusion (图像 + 状态)
        cam_feats_proj = self.in_proj(cam_feats_flat) # [B*L, d_model]
        
        # Query: Image Proj, Key/Value: Lowdim
        q = cam_feats_proj.unsqueeze(0) # [1, B*L, d_model]
        # Lowdim 需要投影吗？cross_attn 内部有 proj_lowdim
        k = lowdim_flat.unsqueeze(0)    # [1, B*L, 14]
        
        fused_flat = self.cross_attn(query=q, key=k, value=k) # [1, B*L, d_model]
        fused_flat = fused_flat.squeeze(0) # [B*L, d_model]

        # 5. 恢复时序结构，送入 Mamba
        x = fused_flat.view(B, L, self.d_model) # [B, L, d_model]

        # 6. Mamba Blocks (Sequence Processing)
        # Mamba 会自动处理这里的序列因果性
        residual = None
        for blk in self.blocks:
        ...
        # 7. Output Projection
        actions_flat = self.out_proj(x) # [B, L, future*act]
        pred_actions = actions_flat.view(B, L, self.future_steps, self.action_dim)
